...
std::string filtersToString(h256Hash const& _fs) {
    ...
    for (h256 const& f : _fs) {
        str << (i++ ? ", " : "");
        if (f == PendingChangedFilter) str << "pending";
        else if (f == ChainChangedFilter) str << "chain";
        else str << f;
    }
    str << "}";
    return str.str();
...
std::ostream& dev::eth::operator<<(std::ostream& _out, ActivityReport const& _r) {
    return (_out << "Since " << toString(_r.since) << " (" << std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - _r.since).count() << "): " << _r.ticks << "ticks");
}

Client::Client(ChainParams const& _params, int _networkID, p2p::Host& _host, std::shared_ptr<GasPricer> _gpForAdoption, fs::path const& _dbPath, fs::path const& _snapshotPath, WithExisting _forceAction, TransactionQueue::Limits const& _l) : Worker("eth", 0),
    ...
    m_preSeal(chainParams().accountStartNonce), m_postSeal(chainParams().accountStartNonce), m_working(chainParams().accountStartNonce) {
...
void Client::init(p2p::Host& _extNet, fs::path const& _dbPath, fs::path const& _snapshotDownloadPath, WithExisting _forceAction, u256 _networkId) {
    ...
    bc().setOnBlockImport([=](BlockHeader const& _info) { if (auto h = m_host.lock()) h->onBlockImported(_info); });

    if (_forceAction == WithExisting::Rescue) bc().rescue(m_stateDB);

    m_gp->update(bc());

    // create Ethereum capability only if we're not downloading the snapshot
    if (_snapshotDownloadPath.empty()) {
        auto ethCapability = make_shared<EthereumCapability>(_extNet.capabilityHost(), bc(), m_stateDB, m_tq, m_bq, _networkId);
    ...
    if (!_snapshotDownloadPath.empty() || importedSnapshotExists) {
        std::shared_ptr<SnapshotStorageFace> snapshotStorage(importedSnapshotExists ? createSnapshotStorage(importedSnapshot) : nullptr);
        auto warpCapability = make_shared<WarpCapability>(_extNet.capabilityHost(), bc(), _networkId, _snapshotDownloadPath, snapshotStorage);
...
ImportResult Client::queueBlock(bytes const& _block, bool _isSafe) {
    if (m_bq.status().verified + m_bq.status().verifying + m_bq.status().unverified > 10000) this_thread::sleep_for(std::chrono::milliseconds(500));
...
void Client::callQueuedFunctions() {
    while (true) {
        function<void()> f;
        DEV_WRITE_GUARDED(x_functionQueue) if (!m_functionQueue.empty()) {
...
u256 Client::networkId() const {
    if (auto h = m_host.lock()) return h->networkId();
    return 0;
}

void Client::setNetworkId(u256 const& _n) {
    if (auto h = m_host.lock()) h->setNetworkId(_n);
}

bool Client::isSyncing() const {
    if (auto h = m_host.lock()) return h->isSyncing();
    return false;
}

bool Client::isMajorSyncing() const {
    if (auto h = m_host.lock()) { return (h->status().state != SyncState::Idle || h->bq().items().first > 10); }
    return false;
}
...
void Client::startedWorking() {
    ...
    DEV_WRITE_GUARDED(x_preSeal) m_preSeal.sync(bc());
    DEV_READ_GUARDED(x_preSeal) {
        DEV_WRITE_GUARDED(x_working) m_working = m_preSeal;
        DEV_WRITE_GUARDED(x_postSeal) m_postSeal = m_preSeal;
    }
}
...
void Client::reopenChain(ChainParams const& _p, WithExisting _we) {
    ...
    if (auto h = m_host.lock()) h->reset();

    startedWorking();
    doWork();

    startWorking();
    if (wasSealing) startSealing();
}

void Client::executeInMainThread(function<void ()> const& _function) {
    DEV_WRITE_GUARDED(x_functionQueue) m_functionQueue.push(_function);
    m_signalled.notify_all();
}

void Client::clearPending() {
    DEV_WRITE_GUARDED(x_postSeal) {
        if (!m_postSeal.pending().size()) return;
        m_tq.clear();
        DEV_READ_GUARDED(x_preSeal) m_postSeal = m_preSeal;
    }
...
void Client::appendFromNewPending(TransactionReceipt const& _receipt, h256Hash& io_changed, h256 _sha3) {
    ...
    for (pair<h256 const, InstalledFilter>& i: m_filters) {
        ...
        if (m.size()) {
            // filter catches them
            for (LogEntry const& l: m) i.second.changes.push_back(LocalisedLogEntry(l));
    ...
    for (pair<h256 const, InstalledFilter>& i: m_filters) {
        ...
        for (size_t j = 0; j < receipts.size(); j++) {
            ...
            if (m.size()) {
                ...
                // filter catches them
                for (LogEntry const& l: m) i.second.changes.push_back(LocalisedLogEntry(l, _block, (BlockNumber)bc().number(_block), transactionHash, j, 0, _polarity));
                io_changed.insert(i.first);
            }
        }
    }
...
void Client::syncBlockQueue() {
    ...
    // The verified blocks list needs to be a shared_ptr since we propagate them on the network thread and import them into our local chain on the client thread.
    ...
    auto h = m_host.lock();
    assert(h); // capability is owned by Host and should be available for the duration of the Client's lifetime
    ...
    if (count) LOG(m_logger) << count << " blocks imported in " << unsigned(elapsed * 1000) << " ms (" << (count / elapsed) << " blocks/s) in #" << bc().number() << " [" << ir.deadBlocks.size() << " dead]";

    if (elapsed > c_targetDurationS * 1.1 && count > c_syncMinBlockCount) m_syncAmount = max(c_syncMinBlockCount, count * 9 / 10);
    else if (count == m_syncAmount && elapsed < c_targetDurationS * 0.9 && m_syncAmount < c_syncMaxBlockCount) m_syncAmount = min(c_syncMaxBlockCount, m_syncAmount * 11 / 10 + 1);
    if (ir.liveBlocks.empty()) return;
    onChainChanged(ir);
}
...
void Client::syncTransactionQueue() {
    ...
    DEV_WRITE_GUARDED(x_working) {
        if (m_working.isSealed()) { ctrace << "Skipping txq sync for a sealed block."; return; }
    ...
    DEV_READ_GUARDED(x_postSeal) for (size_t i = 0; i < newPendingReceipts.size(); i++) appendFromNewPending(newPendingReceipts[i], changeds, m_postSeal.pending()[i].sha3());
...
void Client::onDeadBlocks(h256s const& _blocks, h256Hash& io_changed) {
    ...
    for (auto const& h: _blocks) appendFromBlock(h, BlockPolarity::Dead, io_changed);
}

void Client::onNewBlocks(h256s const& _blocks, h256Hash& io_changed) {
    // remove transactions from m_tq nicely rather than relying on out of date nonce later on.
    for (auto const& h: _blocks) LOG(m_loggerDetail) << "Live block: " << h;
    if (auto h = m_host.lock()) h->noteNewBlocks();
    for (auto const& h: _blocks) appendFromBlock(h, BlockPolarity::Live, io_changed);
}

void Client::resyncStateFromChain() {
    DEV_READ_GUARDED(x_working) if (bc().currentHash() == m_working.info().parentHash()) return;
...
void Client::restartMining() {
    ...
    DEV_READ_GUARDED(x_preSeal) newPreMine = m_preSeal;

    // TODO: use m_postSeal to avoid re-evaluating our own blocks.
    preChanged = newPreMine.sync(bc());

    DEV_READ_GUARDED(x_preSeal) DEV_READ_GUARDED(x_postSeal) if (!preChanged && m_preSeal.author() == m_postSeal.author()) { onTransactionQueueReady(); return; }

    DEV_WRITE_GUARDED(x_preSeal) m_preSeal = newPreMine;
    DEV_WRITE_GUARDED(x_working) m_working = newPreMine;
    DEV_READ_GUARDED(x_postSeal) if (!m_postSeal.isSealed() || m_postSeal.info().hash() != newPreMine.info().parentHash()) for (auto const& t : m_postSeal.pending()) {
        ...
        if (ir != ImportResult::Success) onTransactionQueueReady();
    }
...
//  ctrace << "onChainChanged()";
    ...
    auto h = m_host.lock();
    if (h) h->removeSentTransactions(goodTransactions);
    onNewBlocks(_ir.liveBlocks, changeds);
    if (!isMajorSyncing()) resyncStateFromChain();
    noteChanged(changeds);
...
void Client::startSealing() {
    if (m_wouldSeal == true) return;
    LOG(m_logger) << "Mining Beneficiary: " << author();
    if (author()) {
        m_wouldSeal = true;
        m_signalled.notify_all();
    } else LOG(m_logger) << "You need to set an author in order to seal!";
}
...
void Client::rejigSealing() {
    if ((wouldSeal() || remoteActive()) && !isMajorSyncing()) {
        if (sealEngine()->shouldSeal(this)) {
            ...
            DEV_WRITE_GUARDED(x_working) {
                if (m_working.isSealed()) { LOG(m_logger) << "Tried to seal sealed block..."; return; }
                // TODO is that needed? we have "Generating seal on" below
                LOG(m_loggerDetail) << "Starting to seal block #" << m_working.info().number();
                m_working.commitToSeal(bc(), m_extraData);
            }
            DEV_READ_GUARDED(x_working) {
                DEV_WRITE_GUARDED(x_postSeal) m_postSeal = m_working;
            ...
            if (wouldSeal()) {
                sealEngine()->onSealGenerated([=](bytes const& _header) {
                    ...
                    if (this->submitSealed(_header)) m_onBlockSealed(_header);
                    else LOG(m_logger) << "Submitting block failed...";
                });
                ctrace << "Generating seal on " << m_sealingInfo.hash(WithoutSeal) << " #" << m_sealingInfo.number();
                sealEngine()->generateSeal(m_sealingInfo);
            }
        } else m_wouldButShouldnot = true;
    }
    if (!m_wouldSeal) sealEngine()->cancelGeneration();
}

void Client::noteChanged(h256Hash const& _filters) {
    Guard l(x_filtersWatches);
    if (_filters.size()) LOG(m_loggerWatch) << "noteChanged: " << filtersToString(_filters);
    ...
    for (auto& w: m_watches) if (_filters.count(w.second.id)) {
        ...
        } else if (m_specialFilters.count(w.second.id)) for (h256 const& hash: m_specialFilters.at(w.second.id)) {
            LOG(m_loggerWatch) << "!!! " << w.first << " " << (w.second.id == PendingChangedFilter ? "pending" : w.second.id == ChainChangedFilter ? "chain" : "???");
            w.second.changes.push_back(LocalisedLogEntry(SpecialLogEntry, hash));
        }
    }
    // clear the filters now.
    for (auto& i: m_filters) i.second.changes.clear();
    for (auto& i: m_specialFilters) i.second.clear();
}

void Client::doWork(bool _doWait) {
    bool t = true;
    if (m_syncBlockQueue.compare_exchange_strong(t, false)) syncBlockQueue();
    ...
    tick(); rejigSealing(); callQueuedFunctions();
...
void Client::tick() {
    if (chrono::system_clock::now() - m_lastTick > chrono::seconds(1)) {
        ...
        if (m_report.ticks == 15) LOG(m_loggerDetail) << activityReport();
    }
}
...
void Client::checkWatchGarbage() {
    if (chrono::system_clock::now() - m_lastGarbageCollection > chrono::seconds(5)) {
        ...
        DEV_GUARDED(x_filtersWatches)
            for (auto key: keysOf(m_watches)) if (m_watches[key].lastPoll != chrono::system_clock::time_point::max() && chrono::system_clock::now() - m_watches[key].lastPoll > chrono::seconds(20)) {
                toUninstall.push_back(key);
                LOG(m_loggerDetail) << "GC: Uninstall " << key << " (" << chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - m_watches[key].lastPoll).count() << " s old)";
            }
        for (auto i: toUninstall) uninstallWatch(i);
...
TransactionSkeleton Client::populateTransactionWithDefaults(TransactionSkeleton const& _t) const {
    ...
    if (ret.nonce == Invalid256) ret.nonce = max<u256>(postSeal().transactionsFrom(ret.from), m_tq.maxNonce(ret.from));
    if (ret.gasPrice == Invalid256) ret.gasPrice = gasBidPrice();
    if (ret.gas == Invalid256) ret.gas = defaultTransactionGas;
...
bool Client::submitSealed(bytes const& _header) {
    bytes newBlock;
    {
        UpgradableGuard l(x_working); {
            UpgradeGuard l2(l);
            if (!m_working.sealBlock(_header)) return false;
        }
        DEV_WRITE_GUARDED(x_postSeal) m_postSeal = m_working;
...
void Client::rewind(unsigned _n) {
    ...
    for (unsigned i = 0; i < 10; ++i) {
        u256 n;
        DEV_READ_GUARDED(x_working) n = m_working.info().number();
        if (n == _n + 1) break;
...
h256 Client::importTransaction(Transaction const& _t) {
    ...
    try { Executive(m_postSeal, bc()).initialize(_t); }
    catch (InvalidNonce const& e) {
        // Too low nonce is invalid for sure
        bigint const& req = *boost::get_error_info<errinfo_required>(e), got = *boost::get_error_info<errinfo_got>(e);
        if (req > got) throw;

        // Checking against pending block doesn't take into account transactions from the same sender, that are currently in Transaction Queue.
    ...
    switch (m_tq.import(_t.rlp())) {
        case ImportResult::Success: break;
        case ImportResult::ZeroSignature: BOOST_THROW_EXCEPTION(ZeroSignatureTransaction());
        case ImportResult::OverbidGasPrice: BOOST_THROW_EXCEPTION(GasPriceTooLow());
        case ImportResult::AlreadyKnown: BOOST_THROW_EXCEPTION(PendingTransactionAlreadyExists());
        case ImportResult::AlreadyInChain: BOOST_THROW_EXCEPTION(TransactionAlreadyInChain());
        default: BOOST_THROW_EXCEPTION(UnknownTransactionValidationError());
    }
...
ExecutionResult Client::call(Address const& _from, u256 _value, Address _dest, bytes const& _data, u256 _gas, u256 _gasPrice, BlockNumber _blockNumber, FudgeFactor _ff) {
    ...
    try {
        ...
        t.forceSender(_from);
        if (_ff == FudgeFactor::Lenient) temp.mutableState().addBalance(_from, (u256)(t.gas() * t.gasPrice() + t.value()));
        ret = temp.execute(bc().lastBlockHashes(), t, Permanence::Reverted);
    } catch (...) { cwarn << boost::current_exception_diagnostic_information(); }
    return ret;
}
...
std::tuple<h256, h256, h256> Client::getWork() {
    ...
    if (!sealEngine()->shouldSeal(this)) return std::tuple<h256, h256, h256>();

    // if this request has made us bother to serve work, prep it now.
    if (!oldShould && shouldServeWork()) onPostStateChanged();
    else m_remoteWorking = true; // otherwise, set this to true so that it gets prepped next time.
