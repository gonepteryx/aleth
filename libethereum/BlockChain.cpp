...
static const chrono::system_clock::duration c_collectionDuration = chrono::seconds(60); /// Duration between flushes.
static const unsigned c_collectionQueueSize = 20; /// Length of death row (total time in cache is multiple of this and collection duration).
static const unsigned c_maxCacheSize = 1024 * 1024 * 64; /// Max size, above which we start forcing cache reduction.
static const unsigned c_minCacheSize = 1024 * 1024 * 32; /// Min size, below which we don't bother flushing it.

BlockChain::BlockChain(ChainParams const& _p, fs::path const& _dbPath, WithExisting _we, ProgressCallback const& _pc) : m_lastBlockHashes(new LastBlockHashes(*this)) {
    init(_p);
...
bool BlockChain::open(fs::path const& _path, WithExisting _we) {
    ...
    if (db::isDiskDatabase()) {
        if (!m_dbPaths || m_dbPaths->rootPath() != _path) m_dbPaths = make_unique<DatabasePaths>(_path, m_genesisHash);

        if (_we == WithExisting::Kill) {
            LOG(m_loggerInfo) << "Deleting chain databases. This will require a resync from genesis.";
        ...
        if (!minorVersionBytes.empty()) {
            ...
            if (c_databaseMinorVersion != lastMinor) {
                rebuildNeeded = true;
                LOG(m_loggerInfo) << "Database minor version change detected, the extras and state databases will be rebuilt.";
                LOG(m_loggerInfo) << "Version from " << m_dbPaths->extrasMinorVersionPath() << " (" << lastMinor << ") != Aleth's version (" << c_databaseMinorVersion << ")";
            }
        } else if (fs::exists(m_dbPaths->extrasPath())) {
            LOG(m_loggerInfo) << "Database minor version file not found (" << m_dbPaths->extrasMinorVersionPath() << ") but extras database exists (" << m_dbPaths->extrasPath() << "), assuming extras database needs to be upgraded.";
            rebuildNeeded = true;
        } else { // First launch with new database
            LOG(m_loggerDetail) << "Creating database minor version file: " << m_dbPaths->extrasMinorVersionPath() << " (minor version: " << c_databaseMinorVersion << ")";
    ...
    try {
    ...
    } catch (db::DatabaseError const& ex) { // Determine which database open call failed
        auto const dbPath = !m_blocksDB.get() ? m_dbPaths->blocksPath() : m_dbPaths->extrasPath();
        if (db::isDiskDatabase()) {
            ...
            if (fs::space(m_dbPaths->rootPath()).available < 1024) {
                LOG(m_loggerError) << "Not enough available space found on hard drive. Please free some up and re-run.";
                BOOST_THROW_EXCEPTION(NotEnoughAvailableSpace());
            } else if (dbStatus == db::DatabaseStatus::Corruption) {
                LOG(m_loggerError) << "Database corruption detected. Please see the exception for corruption details. Exception: " << ex.what();
        ...
        LOG(m_loggerError) << "Unknown error occurred when opening database. Exception details: " << ex.what();
        throw;
    }
    ...
    if (_we != WithExisting::Verify && !rebuildNeeded && !details(m_genesisHash)) {
        ...
        m_details[m_genesisHash] = BlockDetails{0 /* number */, genesisHeader.difficulty(), h256{} /* parent */, {} /* children */, genesisBlockBytes.size()};
        auto const genesisDetailsRlp = m_details[m_genesisHash].rlp();
        m_extrasDB->insert(toSlice(m_genesisHash, ExtraDetails), (db::Slice)dev::ref(genesisDetailsRlp));
    ...
    LOG(m_loggerInfo) << "Opened blockchain database. Latest block hash: " << currentHash() << (!rebuildNeeded ? "(rebuild not needed)" : "*** REBUILD NEEDED ***");
    return rebuildNeeded;
}

void BlockChain::open(fs::path const& _path, WithExisting _we, ProgressCallback const& _pc) {
    if (open(_path, _we) || _we == WithExisting::Verify) rebuild(_path, _pc);
}
...
void BlockChain::rebuild(fs::path const& _path, std::function<void(unsigned, unsigned)> const& _progress) {
    if (!db::isDiskDatabase()) {
        LOG(m_loggerWarn) << "In-memory database detected, skipping rebuild (since there's no existing database to rebuild)";
        return;
    }
    ...
    m_extrasDB.reset();
    LOG(m_loggerInfo) << "Renaming extras path " << m_dbPaths->extrasPath() << " to "  << m_dbPaths->extrasTemporaryPath();
    ...
    // first block.
    auto const genesisDetails = BlockDetails{0 /* block number */, s.info().difficulty(), h256{} /* parent */, {} /* children */, m_params.genesisBlock().size()};
    m_details[m_genesisHash] = genesisDetails;
    auto const genesisDetailsRlp = genesisDetails.rlp();
    m_extrasDB->insert(toSlice(m_genesisHash, ExtraDetails), (db::Slice)dev::ref(genesisDetailsRlp));

    LOG(m_loggerInfo) << "Rebuilding the extras and state databases by reimporting blocks 0 -> " << originalNumber << ", this will probably take a while";
...
tuple<ImportRoute, h256s, unsigned> BlockChain::sync(VerifiedBlocks const& _blocks, OverlayDB const& _stateDB) {
    h256s fresh, dead, badBlockHashes;
    Transactions goodTransactions;
    unsigned count = 0;
    for (VerifiedBlock const& block: _blocks) try {
        ...
        ++count;
    } catch (dev::eth::AlreadyHaveBlock const&) { cwarn << "ODD: Import queue contains already imported block"; continue; }
    catch (dev::eth::FutureTime const&) { cwarn << "ODD: Import queue contains a block with future time."; this_thread::sleep_for(chrono::seconds(1)); continue; }
    catch (dev::eth::TransientError const&) { this_thread::sleep_for(chrono::milliseconds(100)); continue; }
...
void BlockChain::insert(VerifiedBlockRef _block, bytesConstRef _receipts, bool _mustBeNew)
{
    ...
    DEV_WRITE_GUARDED(x_details) if (!dev::contains(m_details[_block.info.parentHash()].childHashes, _block.info.hash())) m_details[_block.info.parentHash()].childHashes.push_back(_block.info.hash());
    ...
    try { m_blocksDB->commit(std::move(blocksWriteBatch)); }
    catch (boost::exception const& ex) { cwarn << "Error writing to blockchain database: " << boost::diagnostic_information(ex) << "Fail writing to blockchain database. Bombing out."; exit(-1); }
    try { m_extrasDB->commit(std::move(extrasWriteBatch)); }
    catch (boost::exception const& ex) { cwarn << "Error writing to extras database: " << boost::diagnostic_information(ex) << "Fail writing to extras database. Bombing out."; exit(-1); }
}

ImportRoute BlockChain::import(VerifiedBlockRef const& _block, OverlayDB const& _db, bool _mustBeNew) { //@tidy This is a behemoth of a method - could do to be split into a few smaller ones.
    ImportPerformanceLogger performanceLogger;

    if (_mustBeNew) checkBlockIsNew(_block); // Check block doesn't already exist first!

    // Work out its number as the parent's number + 1
    if (!isKnown(_block.info.parentHash(), false)) { // doesn't have to be current.
        LOG(m_logger) << _block.info.hash() << " : Unknown parent " << _block.info.parentHash();
        BOOST_THROW_EXCEPTION(UnknownParent() << errinfo_hash256(_block.info.parentHash())); // We don't know the parent (yet) - discard for now. It'll get resent to us if we find out about its ancestry later on.
    }

    auto pd = details(_block.info.parentHash());
    if (!pd) { // == number invalid/not known
        auto pdata = pd.rlp();
        LOG(m_loggerError) << "Details is returning false despite block known: " << RLP(pdata);
        auto parentBlock = block(_block.info.parentHash());
        LOG(m_loggerError) << "isKnown: " << isKnown(_block.info.parentHash()) << "last/number: " << m_lastBlockNumber << " " << m_lastBlockHash << " " << _block.info.number() << "Block: " << BlockHeader(&parentBlock) << "RLP: " << RLP(parentBlock) << "DATABASE CORRUPTION: CRITICAL FAILURE";
        exit(-1);
    }
    ...
    LOG(m_loggerDetail) << "Attempting import of block " << _block.info.hash() << " (#" << _block.info.number() << ") ...";
    performanceLogger.onStageFinished("preliminaryChecks");

    BlockReceipts br;
    u256 td;
    try {
        // Check transactions are valid and that they result in a state equivalent to our state_root. Get total difficulty increase and update state, checking it.
        Block s(*this, _db);
        auto tdIncrease = s.enactOn(_block, *this);

        for (unsigned i = 0; i < s.pending().size(); ++i) br.receipts.push_back(s.receipt(i));
        ...
        cwarn << "*** BadRoot error! Trying to import" << _block.info.hash() << "needed root" << *boost::get_error_info<errinfo_hash256>(ex) << _block.info;
        BOOST_THROW_EXCEPTION(TransientError()); // Attempt in import later.
    } catch (Exception& ex) { addBlockInfo(ex, _block.info, _block.block.toBytes()); throw; }
...
ImportRoute BlockChain::insertWithoutParent(bytes const& _block, bytesConstRef _receipts, u256 const& _totalDifficulty) {
    ...
    checkBlockIsNew(block); // Check block doesn't already exist first!
...
void BlockChain::checkBlockTimestamp(BlockHeader const& _header) const {
  if (_header.timestamp() > utcTime() && !m_params.allowFutureBlocks) { // Check it's not crazy
    LOG(m_loggerDetail) << _header.hash() << " : Future time " << _header.timestamp() << " (now at " << utcTime() << ")";
    BOOST_THROW_EXCEPTION(FutureTime()); // Block has a timestamp in the future. This is no good.
  }
}
...
ImportRoute BlockChain::insertBlockAndExtras(VerifiedBlockRef const& _block, bytesConstRef _receipts, u256 const& _totalDifficulty, ImportPerformanceLogger& _performanceLogger) {
    std::unique_ptr<db::WriteBatchFace> blocksWriteBatch = m_blocksDB->createWriteBatch(), extrasWriteBatch = m_extrasDB->createWriteBatch();
    h256 newLastBlockHash = currentHash();
    unsigned newLastBlockNumber = number();
    try {
        // ensure parent is cached for later addition.
        // TODO: this is a bit horrible would be better refactored into an enveloping UpgradableGuard together with an "ensureCachedWithUpdatableLock(l)" method. This is safe in practice since the caches don't get flushed nearly often enough to be done here.
        details(_block.info.parentHash());
        DEV_WRITE_GUARDED(x_details) m_details[_block.info.parentHash()].childHashes.push_back(_block.info.hash());

        _performanceLogger.onStageFinished("collation");

        blocksWriteBatch->insert(toSlice(_block.info.hash()), db::Slice(_block.block));
        DEV_READ_GUARDED(x_details) extrasWriteBatch->insert(toSlice(_block.info.parentHash(), ExtraDetails), (db::Slice)dev::ref(m_details[_block.info.parentHash()].rlp()));

        BlockDetails const details{static_cast<unsigned>(_block.info.number()), _totalDifficulty, _block.info.parentHash(), {} /* children */, _block.block.size()};
        extrasWriteBatch->insert(toSlice(_block.info.hash(), ExtraDetails), (db::Slice)dev::ref(details.rlp()));

        BlockLogBlooms blb;
        for (auto i: RLP(_receipts)) blb.blooms.push_back(TransactionReceipt(i.data()).bloom());
    ...
    h256s route;
    bool isImportedAndBest = false; // This might be the new best block...
    h256 common, last = currentHash();
    if (_totalDifficulty > details(last).totalDifficulty || (m_sealEngine->chainParams().tieBreakingGas && _totalDifficulty == details(last).totalDifficulty && _block.info.gasUsed() > info(last).gasUsed())) {
        // don't include bi.hash() in treeRoute, since it's not yet in details DB... just tack it on afterwards.
        unsigned commonIndex;
        tie(route, common, commonIndex) = treeRoute(last, _block.info.parentHash());
        route.push_back(_block.info.hash());

        // Most of the time these two will be equal - only when we're doing a chain revert will they not be
        if (common != last) DEV_READ_GUARDED(x_lastBlockHash) clearCachesDuringChainReversion(number(common) + 1);

        // Go through ret backwards (i.e. from new head to common) until hash != last.parent and update m_transactionAddresses, m_blockHashes
        for (auto i = route.rbegin(); i != route.rend() && *i != common; ++i) {
            BlockHeader tbi = (*i == _block.info.hash()) ? _block.info : BlockHeader(block(*i));
            ...
            h256s alteredBlooms;
            {
                ...
                for (unsigned level = 0, index = (unsigned)tbi.number(); level < c_bloomIndexLevels; level++, index /= c_bloomIndexSize) blocksBlooms(chunkId(level, index / c_bloomIndexSize));

                WriteGuard l(x_blocksBlooms);
                for (unsigned level = 0, index = (unsigned)tbi.number(); level < c_bloomIndexLevels; level++, index /= c_bloomIndexSize) {
                    unsigned i = index / c_bloomIndexSize, o = index % c_bloomIndexSize;
            ...
            //h256s newTransactionAddresses;
            {
                ...
                for (ta.index = 0; ta.index < blockRLP[1].itemCount(); ++ta.index) extrasWriteBatch->insert(toSlice(sha3(blockRLP[1][ta.index].data()), ExtraTransactionAddress), (db::Slice)dev::ref(ta.rlp()));
            }

            // Update database with them.
            ReadGuard l1(x_blocksBlooms);
            for (auto const& h: alteredBlooms) extrasWriteBatch->insert(toSlice(h, ExtraBlocksBlooms), (db::Slice)dev::ref(m_blocksBlooms[h].rlp()));
        ...
        LOG(m_logger) << "   Imported and best " << _totalDifficulty << " (#" << _block.info.number() << "). Has " << (details(_block.info.parentHash()).childHashes.size() - 1) << " siblings. Route: " << route;
    } else { LOG(m_loggerDetail) << "   Imported but not best (oTD: " << details(last).totalDifficulty << " > TD: " << _totalDifficulty << "; " << details(last).number << ".." << _block.info.number() << ")"; }

    try { m_blocksDB->commit(std::move(blocksWriteBatch)); } catch (boost::exception& ex) { cwarn << "Error writing to blockchain database: " << boost::diagnostic_information(ex) << "Fail writing to blockchain database. Bombing out."; exit(-1); }
    try { m_extrasDB->commit(std::move(extrasWriteBatch)); } catch (boost::exception& ex) { cwarn << "Error writing to extras database: " << boost::diagnostic_information(ex) << "Fail writing to extras database. Bombing out."; exit(-1); }

    if (m_lastBlockHash != newLastBlockHash) DEV_WRITE_GUARDED(x_lastBlockHash) {
        m_lastBlockHash = newLastBlockHash;
        m_lastBlockNumber = newLastBlockNumber;
        try { m_extrasDB->insert(db::Slice("best"), db::Slice((char const*)&m_lastBlockHash, 32)); }
    ...
    if (!route.empty()) noteCanonChanged();
    if (isImportedAndBest && m_onBlockImport) m_onBlockImport(_block.info);

    h256s fresh, dead;
    bool isOld = true;
    for (auto const& h: route)
        if (h == common) { isOld = false; }
        else if (isOld) { dead.push_back(h); }
        else fresh.push_back(h);
...
void BlockChain::noteUsed(h256 const& _h, unsigned _extra) const {
    ...
    if (m_cacheUsage[1].count(id)) m_cacheUsage[1].erase(id);
    else m_inUse.insert(id);
}
...
void BlockChain::updateStats() const {
    ...
    DEV_READ_GUARDED(x_blocks) for (auto const& i: m_blocks) m_lastStats.memBlocks += i.second.size() + 64;
...
void BlockChain::garbageCollect(bool _force) {
    ...
    if (!_force && chrono::system_clock::now() < m_lastCollection + c_collectionDuration && m_lastStats.memTotal() < c_maxCacheSize) return;
    if (m_lastStats.memTotal() < c_minCacheSize) return;
    ...
    m_blocksDB->forEach([this](db::Slice const& _key, db::Slice const& /* _value */) {
        if (_key.size() == 32)
        {
            ...
            if (p != h256() && p != m_genesisHash) { // TODO: for some reason the genesis details with the children get squished. not sure why.
                auto dp = details(p);
                if (asserts(contains(dp.childHashes, h))) cnote << "Apparently the database is corrupt. Not much we can do at this stage...";
                if (assertsEqual(dp.number, dh.number - 1)) cnote << "Apparently the database is corrupt. Not much we can do at this stage...";
            }
        }
        return true;
    });
...
void BlockChain::clearCachesDuringChainReversion(unsigned _firstInvalid) {
    ...
    DEV_WRITE_GUARDED(x_blockHashes) for (auto i = _firstInvalid; i < end; ++i) m_blockHashes.erase(i);
    DEV_WRITE_GUARDED(x_transactionAddresses) m_transactionAddresses.clear(); // TODO: could perhaps delete them individually?
...
vector<unsigned> BlockChain::withBlockBloom(LogBloom const& _b, unsigned _earliest, unsigned _latest, unsigned _level, unsigned _index) const {
    ...
    for (unsigned o = obegin; o < oend; ++o)
        if (bb.blooms[o].contains(_b)) {
            ...
            if (_level > 0) { ret += withBlockBloom(_b, _earliest, _latest, _level - 1, o + _index * c_bloomIndexSize); }
            else ret.push_back(o + _index * c_bloomIndexSize);
        }
    return ret;
}
...
h256Hash BlockChain::allKinFrom(h256 const& _parent, unsigned _generations) const {
    ...
    for (unsigned i = 0; i < _generations && p != m_genesisHash; ++i, p = details(p).parentHash) {
        ...
        auto b = block(p);
        for (auto i: RLP(b)[2]) ret.insert(sha3(i.data()));
    }
    return ret;
}

bool BlockChain::isKnown(h256 const& _hash, bool _isCurrent) const {
    if (_hash == m_genesisHash) return true;

    DEV_READ_GUARDED(x_blocks) if (!m_blocks.count(_hash) && !m_blocksDB->exists(toSlice(_hash))) { return false; }
    DEV_READ_GUARDED(x_details) if (!m_details.count(_hash) && !m_extrasDB->exists(toSlice(_hash, ExtraDetails))) { return false; }
//  return true;
...
bytes BlockChain::block(h256 const& _hash) const {
    if (_hash == m_genesisHash) return m_params.genesisBlock();
    ...
    if (d.empty()) { cwarn << "Couldn't find requested block:" << _hash; return bytes(); }
...
bytes BlockChain::headerData(h256 const& _hash) const  {
    if (_hash == m_genesisHash) return m_genesisHeaderBytes;

    {
        ReadGuard l(x_blocks);
        auto it = m_blocks.find(_hash);
        if (it != m_blocks.end()) return BlockHeader::extractHeader(&it->second).data().toBytes();
    }
...
VerifiedBlockRef BlockChain::verifyBlock(bytesConstRef _block, std::function<void(Exception&)> const& _onBad, ImportRequirements::value _ir) const {
    ...
    try {
        ...
        if (!!(_ir & ImportRequirements::Parent)) {
            ...
            if (parentHeader.empty()) BOOST_THROW_EXCEPTION(InvalidParentHash() << errinfo_required_h256(h.parentHash()) << errinfo_currentNumber(h.number()));
...
void BlockChain::setChainStartBlockNumber(unsigned _number) {
    ...
    if (!hash) BOOST_THROW_EXCEPTION(UnknownBlockNumber());

    try { m_extrasDB->insert(c_sliceChainStart, db::Slice(reinterpret_cast<char const*>(hash.data()), h256::size)); }
    catch (boost::exception const& ex) { BOOST_THROW_EXCEPTION(FailedToWriteChainStart() << errinfo_hash256(hash) << boost::errinfo_nested_exception(boost::copy_exception(ex))); }
}
